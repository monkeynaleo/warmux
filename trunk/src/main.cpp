                           ...
                           long_options, &option_index)) != -1)
    {
      switch (c)
        ...
        case 'h':
          ...
          printf("\nWith :\n");
          printf(" <debug_msg> ::= { action | action_handler | action_handler.menu | ai | ai.move | body | body.state | body_anim | bonus | box | camera.tracking | character | damage | downloader | explosion | game | game.endofturn | game_mode | game.pause | game.statechange | ghost | grapple.hook | grapple.node | ground_generator.element | index_server | jukebox | jukebox.play | lst_objects | map | map.load | map.random | menu | mine | mouse | network | network.traffic | network.turn_master | physical | physical.mem | physic.compute | physic.fall | physic.move | physic.overlapping | physic.pendulum | physic.physic | physic.position | physic.state | socket | sprite | team | test_rectangle | weapon.change | weapon.handposition | weapon.projectile | weapon.shoot | wind }\n");
          exit(0);
          break;
        case 'v':
