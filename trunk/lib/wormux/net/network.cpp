...
bool WNet::Server_SendStatusToIndexServer(WSocket& index_server_socket,
					  const std::string& game_name,
					  bool passwd,
					  int port)
{
  ...
  used += Batch(buffer, (int)TS_MSG_HOSTING);
  // Reserve 4 bytes for the total message length.
  used += 4;
  used += Batch(buffer+used, game_name);
  used += Batch(buffer+used, (int)passwd);
  used += Batch(buffer+used, port);

  FinalizeBatch(buffer, used);
  bool r = index_server_socket.SendBuffer(buffer, used);
  if (!r)
    return false;

  r = index_server_socket.ReceiveStr(ack, 5);
  if (r && ack == "OK")
    return true;
...
bool WNet::KeepConnectionWithIndexServer(WSocket& index_server_socket)
{
  if (!index_server_socket.IsReady(100))
    return true;

  int msg_id;
  bool r;
  r = index_server_socket.ReceiveInt(msg_id);
  if (!r || msg_id != TS_MSG_PING)
    return false;

  uint used = 0;
  char buffer[16];
  used += Batch(buffer, (int)TS_MSG_PONG);
  FinalizeBatch(buffer, used);
  r = index_server_socket.SendBuffer(buffer, used);
  if (!r)
    return false;
