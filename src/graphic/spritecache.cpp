...
void SpriteFrameCache::CreateRotationCache(Surface &surface, unsigned int cache_size, bool smooth)
{
  ...
  for(unsigned int i=1 ; i< cache_size ; i++){
    Double angle = 2* PI * (1 /* to inverte rotation angle */ - static_cast<Double>(i) / static_cast<Double>(cache_size));
...
Surface SpriteFrameCache::GetFlippedSurfaceForAngle(Double angle) const
{
  ...
  while(angle_tmp >= 2 * PI)
    angle_tmp -= 2 * PI;
  while(angle_tmp < 0.0)
    angle_tmp += 2 * PI;
  int index = static_cast<uint>(angle_tmp*static_cast<Double>(rotated_flipped_surface.size()) / (2*PI));
...
Surface SpriteFrameCache::GetSurfaceForAngle(Double angle) const
{
  ...
  unsigned int index = static_cast<uint>(angle_tmp*static_cast<Double>(rotated_surface.size()) / (2*PI));
...
void SpriteFrameCache::CreateFlippingCache(Surface &surface, bool smooth)
{
  ...
  if (use_rotation)
  {
    ...
    {
      Double angle = 2 * PI * (1 - (Double) i / (Double) n);
